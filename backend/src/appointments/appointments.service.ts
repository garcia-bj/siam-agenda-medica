...
import type { AppointmentsQueryDto } from './dto/appointments-query.dto.js';
...
export class AppointmentsService {
  ...
  async list(query: AppointmentsQueryDto): Promise<{ data: AppointmentResponse[] }> {
    ...
    if (query.date) {
      const start = DateTime.fromISO(query.date, { zone: this.scheduleService.clinicTz }).startOf('day');
      if (!start.isValid) throw new ApiException(400, 'VALIDATION_ERROR', 'Fecha inválida. Debe tener formato YYYY-MM-DD');
      const end = start.plus({ days: 1 });
      where.startTime = { gte: start.toJSDate(), lt: end.toJSDate() };
    }
    ...
    return { data: appointments.map((appointment) => this.toResponse(appointment)) };
  }
  ...
  async cancel(id: string): Promise<void> {
    ...
    if (appointment.status === 'CANCELLED') {
      throw new ApiException(409, 'ALREADY_CANCELLED', 'La cita ya está cancelada');
    }
  ...
  async reschedule(id: string, startTime: string): Promise<AppointmentResponse> {
    const appointment = await this.prisma.appointment.findUnique({ where: { id } });
    if (!appointment) throw new ApiException(404, 'NOT_FOUND', 'La cita no existe');
    if (appointment.status === 'CANCELLED') {
      throw new ApiException(409, 'ALREADY_CANCELLED', 'La cita ya está cancelada');
    ...
    const start = this.scheduleService.validateSlot(startTime);
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
        throw new ApiException(409, 'SLOT_TAKEN', `El horario ya está ocupado para ${SPECIALTY_LABELS[appointment.specialty as Specialty]}`);
      }
      throw error;
    }
  }
